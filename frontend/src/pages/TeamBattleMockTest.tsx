...
const TeamBattleMockTest = () => {
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        toast.warning("Please don't switch tabs during the test!");
      }
    };
  ...
  const fetchLiveScores = async () => {
    ...
    const { data: challenge } = await supabase
      .from("team_challenges")
      .select("challenger_team, opponent_team")
      .eq("challenge_id", challengeId)
      .single();

    if (!challenge) return;

    const teamIds = [challenge.challenger_team, challenge.opponent_team].filter(Boolean);
    ...
    for (const teamId of teamIds) {
      ...
      scores.push({
        ...
        members_submitted: results?.length || 0,
  ...
  const handleSubmitTest = async () => {
    if (!userId || !userTeamId || !challengeId) {
    ...
    try {
      ...
      // Save to team_challenge_results
      ...
      // Update team challenge scores
      await updateTeamScores();

      toast.success(`Test submitted! Your score: ${totalScore}/300`);
      navigate("/team", { state: { showResults: true } });
    } catch (err) {
  ...
  const updateTeamScores = async () => {
    if (!challengeId || !userTeamId) return;

    // Get all results for this challenge and team
    const { data: results } = await supabase
      .from("team_challenge_results")
      .select("score")
      .eq("challenge_id", challengeId)
    ...
    // Update the challenge with the team's average score
    const isChallenger = challengeData?.challenger_team === userTeamId;
    
    await supabase
      .from("team_challenges")
      .update(isChallenger ? { challenger_score: avgScore } : { opponent_score: avgScore })
      .eq("challenge_id", challengeId);
  };
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Loading team battle...</p>
        </div>
      </div>
    );
  }
      ...
      {screenshotBlocked && (
        <div className="fixed inset-0 z-[100] bg-black flex items-center justify-center">
          <div className="text-center text-white">
            ...
            <p className="text-muted-foreground">Screenshots are not allowed during the test.</p>
          </div>
        </div>
      )}

      {/* Fixed Header with Timer and Live Scores */}
            ...
            {liveScores.map((score) => (
              <div
                ...
                className={`px-4 py-1 rounded-full text-sm font-medium ${
                  score.team_id === userTeamId
                    ? "bg-green-500/20 text-green-500 border border-green-500/50"
                    : "bg-muted text-muted-foreground"
                }`}
              >
                <Trophy className="w-3 h-3 inline mr-1" />
                {score.team_name}: {score.total_score} ({score.members_submitted}/{score.total_members})
              </div>
            ))}
          </div>
        </div>
          ...
          >
            ...
            >
              ...
              <p className="text-muted-foreground mb-6">
                Are you sure you want to submit the test? Your score will be added to your team's total.
              </p>
...
export default TeamBattleMockTest;
